export class AudioSystem {
  ...
  // Play simple synth BGM based on mood
  playTheme(mood) {
  ...
  _getChordForMood(mood) {
    switch(mood) {
      case 'calm': return [220, 277.18, 329.63]; // A major
      case 'suspense': return [196, 233.08, 293.66]; // G minor
      case 'tense': return [110, 130.81, 164.81]; // Low A minor
      case 'sad': return [174.61, 207.65, 261.63]; // F minor
      case 'ending': return [261.63, 329.63, 392.00, 523.25]; // C major 7
      default: return [220, 261.63, 329.63]; // A minor
    }
  }

  // Sound effects
  playClick() {
    this._playTone(600, 'sine', 0.05, 0.1);
  }

  playType() {
    this._playTone(800 + Math.random()*200, 'square', 0.02, 0.05);
  }
